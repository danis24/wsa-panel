  ...
  <div class="card-body">
    <form @submit.prevent="actionLogin">
      <div class="form-group">
        ...
        <input type="text" class="form-control" name="username" placeholder="Username.." aria-autocomplete="list" v-model="login.username">
      </div>

      <div class="form-group">
        <label class="form-label">Password</label>
        <input type="password" class="form-control" name="password" placeholder="Password.." aria-autocomplete="list" v-model="login.password">
      </div>

      <div class="form-group">
        <label class="form-label">2FA (if enable)</label>
        <input type="text" class="form-control" name="otp" placeholder="2FA.." aria-autocomplete="list" v-model="login.otp">
      </div>
    ...
    actionLogin: function () {
      ...
      this.axios.post(basUrl, this.login).then((response) => {
        ...
        if (response.data.LoginInvalid == 1) {
          loader.hide();
          let toast = this.$toasted.show("Username or Password is Incorrect !!", {
            theme: "toasted-primary",
            position: "top-left",
            duration: 5000
          });
